...
type DbPnrrClarification = {
  ...
  status: PnrrClarification["status"];
...
type DbFmAction = {
  ...
  action_type: string;
  label: string;
  transmitted_status: FmAction["transmittedStatus"];
  date_sent: string | null;
  notes: string | null;
  updated_at: string;
};
...
type DbFmAddendum = {
  ...
  reason: string;
  budget_change_needed: FmAddendum["budgetChangeNeeded"];
  procurement_plan_change_needed: FmAddendum["procurementPlanChangeNeeded"];
  activities_term_change_needed: FmAddendum["activitiesTermChangeNeeded"];
  documents_prepared: FmAddendum["documentsPrepared"];
  platform_status: FmAddendum["platformStatus"];
  date_sent: string | null;
  notes: string | null;
  updated_at: string;
};
  ...
  try {
    ...
    const [companiesRes, projectsRes, pnrrRes, fmActionsRes, fmAddendaRes, notesRes, activityRes] = await Promise.all([
      ...
      supabase
        ...
        .select("id, project_id, company_id, subject, request_type, priority, date_received, response_deadline, status, date_sent, is_answer_sent, responsible, notes, created_at, updated_at"),
      supabase
        .from("fm_actions")
